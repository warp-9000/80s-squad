...
> Enable McClane to skip issues requiring capabilities the current machine lacks.

## Overview

When running Squad across multiple machines (laptops, DevBoxes, GPU servers, Kubernetes nodes), each machine has different tooling. The capability system lets you declare what each machine can do, and McClane automatically routes work accordingly.
...
McClane will log skipped issues:
```
...
1. McClane loads `machine-capabilities.json` at startup
2. For each open issue, McClane extracts `needs:*` labels
